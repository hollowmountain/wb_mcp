...
import type { ModelKey } from './higgsfield.js';

/**
 * Согласованный план генерации.
 *
 * Зачем он вообще. Генерация тратит настоящие деньги и почти всегда выходит
 * не с первого раза: модель дорисовывает свои надписи, меняет форму упаковки,
 * упрощает сцену. Поэтому между «человек сказал, чего хочет» и «списались
...
export interface Plan {
    ...
    imageUrl?: string;
    seconds?: number;
    /** Оценка на момент согласования, чтобы показать её же при запуске. */
    usd: number;
    credits: number;
...
/** План живёт полчаса: за это время цена не успеет измениться, а забытый план протухнет. */
