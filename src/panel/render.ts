...
<p class="muted small">Записывается только имя инструмента, кабинет, исход и длительность. Ни запросов, ни ответов, ни текстов переписки здесь нет. «Отказ» — когда человеку не хватило прав: по этой колонке видно, кому чего недостаёт для работы. Записи хранятся неделю и удаляются сами.</p>
