...
2. ИСХОДНОЕ ФОТО. Назовите nmID товара и кабинет — коннектор возьмёт снимок
   из карточки Wildberries сам. Прямую ссылку принимаем только на витрины
   площадок. Без исходника модель нарисует похожий предмет, а не ваш.
3. ВЫБЕРИТЕ КАДР С ЧИСТЫМ ТОВАРОМ. В карточках первое фото обычно уже с
   инфографикой: модель скопирует её вёрстку и повторит надписи бессмыслицей.
   Спросите, на каком из фото товар снят без наложенного текста, и берите
   его — параметр photo. Подтёки геля и блики поверх букв тоже портят: с
   такого кадра буквы на этикетке выходят смазанными.
