...
export function registerOnecTools(server: McpServer, actor: Actor): void {
    ...
    server.registerTool(
        ...
        guarded('onec_stock_value', async (args, extra) => {
            ...
            if (p.noVat > p.sum) {
                // Утверждать, что это ошибка учёта, нельзя: проверено на живых
                // строках, и объяснение не сошлось. Контрагенты здесь — юрлица
                // Wildberries и Ozon, то есть наш товар на их складах. «Сумма»
                // там ведёт себя как себестоимость (около 44 ₽ за штуку,
                // одинаково на обеих площадках), а вторая величина даёт
                // ~1200 ₽ при рознице в 555. Что она означает, из API не видно.
                head.push(
                    '',
                    'По товарам на складах маркетплейсов вторая величина ведёт себя не как',
                    '«без НДС»: она больше первой. Что именно там учитывается, по данным',
                    'коннектора не определить — смотреть надо в самой 1С. Поэтому две части',
                    'не свожу в один итог. По своим складам всё обычно.'
                );
            } else {
