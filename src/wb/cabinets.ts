...
const BUCKET_LIMITS: Record<BucketKey, [capacity: number, perSecond: number]> = {
    ...
    content: [10, 1.5],
    prices: [5, 1],
    // Самая жёсткая область. WB сам называет условия в заголовках ответа 429:
    // X-RateLimit-Limit=1, X-RateLimit-Retry=309 — то есть один запрос в пять
    // минут, а не в минуту, как я решил сначала. Всплеска нет.
    statistics: [1, 1 / 300],
    // Отчёт об остатках требует трёх-четырёх запросов подряд (создать задачу,
    // опросить статус, забрать). Проба 02.09.2026: четыре запроса за 15 секунд
    // прошли без 429, поэтому не душим до одного в минуту, как статистику.
    analytics: [5, 1 / 6],
    supplies: [5, 1],
    finance: [5, 1]
};
